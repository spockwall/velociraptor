...
## Executor (order REST gateway)

The `executor` crate is the order-side counterpart to `orderbook_server`. It accepts msgpack `OrderRequest` frames over a ZMQ ROUTER socket and dispatches them to per-exchange REST clients (currently **Kalshi** and **Polymarket CLOB v2**). The trading engine connects a `DEALER` socket and gets typed `OrderResponse` replies.
...
The credentials file is checked for `chmod 600` at startup; pass `--skip-chmod-check` in dev.

### Action coverage

`OrderAction` variants and their REST mapping:

| Action          | Kalshi                                    | Polymarket v2          |
|-----------------|-------------------------------------------|------------------------|
...
- **Kalshi**: `"{ticker}.YES"` or `"{ticker}.NO"` — the suffix selects yes/no contract side.
- **Polymarket**: token-id string (decimal) as `symbol`.
...
| `executor:backend_heartbeat`     | Backend writes unix-secs every 5s; if stale >30s, dead-man engages.|
| `executor:log`                   | Append-only audit stream (XADD, MAXLEN ~ 100k by default).          |
...
redis-cli XLEN executor:log              # how many audit entries pending
redis-cli XRANGE executor:log - + COUNT 5
```

### Audit trail

Every request and response is dual-sinked:

1. Redis stream `executor:log` (XADD `MAXLEN ~ <cap>`).
2. `./data/executor/{date}.mpack` — length-prefixed msgpack frames, identical bytes to the stream payload.

Each entry carries `seq` and `prev_hash` (BLAKE3 over the prior entry's encoded bytes), so tampering between executor and backend is detectable. Critical entries (errors, kill-switch transitions, cancel-all triggers) are fsynced immediately; routine entries ride the buffer.

### Risk gates

Pre-flight checks (run in order, fail-fast) for every `Place` / batch leg:
...
Failures return `OrderError::RiskRejected { rule, detail }` — the engine sees the typed error directly. Counters are maintained in-process and reconciled against `get_orders()` periodically.

### Tests

```bash
cargo test -p executor                              # unit + ZMQ round-trip
cargo test -p executor --features integration_tests # adds live Kalshi demo / Polymarket testnet (creds required)
```
