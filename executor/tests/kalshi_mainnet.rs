...
//!
//! Credentials: `$KALSHI_CREDENTIALS_FILE`, else the first existing of
//! `credentials/{prod,dev}/kalshi.yaml`. When no usable credentials are
//! found every test SKIPS (prints a note and returns) so plain
//! `cargo test` stays green on machines without Kalshi creds. Auth is local
...
/// Credentials file: `$KALSHI_CREDENTIALS_FILE`, else the first of
/// `credentials/{prod,dev}/kalshi.yaml` that exists.
fn creds_path() -> Option<std::path::PathBuf> {
    if let Ok(p) = std::env::var("KALSHI_CREDENTIALS_FILE") {
        return Some(p.into());
    }
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .to_path_buf();
    ["credentials/prod/kalshi.yaml", "credentials/dev/kalshi.yaml"]
        .iter()
        .map(|rel| root.join(rel))
        .find(|p| p.exists())
}

/// Build a `KalshiRestClient`, or `None` (the test skips) when no usable
/// credentials are present. Sync — RSA-PSS auth is local, no `authenticate()`.
fn build_client() -> Option<KalshiRestClient> {
    let Some(path) = creds_path() else {
        eprintln!(
            "kalshi_mainnet: no credentials (credentials/{{prod,dev}}/kalshi.yaml \
             or $KALSHI_CREDENTIALS_FILE) — skipping"
        );
        return None;
    };
    let Some(creds) = KalshiCredentials::try_load(&path) else {
        eprintln!(
            "kalshi_mainnet: no usable `kalshi:` section in {} — skipping",
            path.display()
        );
        return None;
    };
    if creds.api_key.is_empty() || creds.secret.is_empty() {
        eprintln!(
            "kalshi_mainnet: api_key/secret empty in {} — skipping",
            path.display()
        );
        return None;
    }
