...
/// Exchange-reported execution detail captured at order-ack time.
///
/// Mirrors Polymarket CLOB v2 `PostOrderResponse`: the amounts that were
/// immediately matched, the chain transaction hashes, the maker trade ids,
/// and the raw success/error envelope. Amounts are carried as `f64` so the
/// ZMQ/msgpack protocol stays self-describing without a `Decimal` dependency.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Default)]
pub struct FillInfo {
    /// Maker amount filled (Polymarket `making_amount`).
    pub making_amount: f64,
    /// Taker amount filled (Polymarket `taking_amount`).
    pub taking_amount: f64,
    /// Whether the exchange accepted the order.
    pub success: bool,
    /// Exchange error message, if any (empty string is normalized to `None`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub error_msg: Option<String>,
    /// On-chain settlement transaction hashes (hex, `0x`-prefixed).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub transaction_hashes: Vec<String>,
    /// Exchange-side trade ids generated by this order.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
