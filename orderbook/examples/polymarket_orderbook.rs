...
//!
//! ```bash
//! cargo run --example polymarket_orderbook -- --config configs/polymarket.yaml
...
fn spawn_scheduler(
    ...
    tokio::spawn(async move {
        ...
        if market.interval_secs == 0 {
            ...
            if let Some(task) =
                MarketTask::spawn(&market, base_slug.clone(), base_slug.clone(), store, depth).await
            {
        ...
        loop {
            ...
            let next_task =
                MarketTask::spawn(&market, base_slug.clone(), next_slug, store.clone(), depth)
                    .await;
